...
const idsExceptSelf = ( data:string[], currentUserId:string ) =>(
  data.filter( ( id:string )=>id !== currentUserId )
)

const updateUserConversation = ( convId:string, data:string[], currentUserId:string ) =>{
  return addConvId( convId,
    idsExceptSelf( data, currentUserId ) )
}
...
const getConversation = async ( req: Request, res: Response ) => {
  ...
  const isUserInParticipants = conv.participants.some( ( userId:string ) => userId === req.currentUser._id )
